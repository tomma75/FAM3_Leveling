    ...
    def startLeveling(self):
        ...
        try:
            ...
            if len(list_masterFile) > 0 :
                ...
                ## KSM ADD ST ##
                ## 3차 코드수정 ##
                df_PowerATE = pd.read_excel(list_masterFile[9],header=2)
                ...
                for i in df_addSmtAssyPower.index:
                    ...
                    if str(df_addSmtAssyPower['MSCODE'][i])[:4] in dict_MODEL_TE.keys():
                        if str(df_addSmtAssyPower['긴급오더'][i]) == '대상':
                            ...
                            if dict_MODEL_Ra[str(df_addSmtAssyPower['MSCODE'][i])[:4]] < 0 : t = 1
                            if powerOrderCnt_copy < 0 : 
                                t = 2
                                if dict_MODEL_Ra[str(df_addSmtAssyPower['MSCODE'][i])[:4]] < 0 : t = 1
                            if dict_MODEL_Ra[str(df_addSmtAssyPower['MSCODE'][i])[:4]] < 0 or powerOrderCnt_copy < 0:
                                # if t == 0:
